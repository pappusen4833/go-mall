...
func (d *Gen) GetTableColumns() vo.ResultList {
	...
	global.DB.Where("table_id = ?", data.Id).Find(&list)
...
func (d *Gen) Preview() map[string]interface{} {
	...
	if err != nil {
		global.LOG.Error(err, vueTemp)
	}
	jsTemp, err := template.ParseFiles("templates/js.go.template")
	if err != nil {
		global.LOG.Error(err, jsTemp)
	}
	var (
		data models.SysTables
		list []models.SysColumns
	)
	global.DB.Where("table_name = ?", d.Name).First(&data)
	global.DB.Where("table_id = ?", data.Id).Find(&list)
	data.Columns = list
...
func (d *Gen) GenCode() {
	...
	//创建目录
	file.MkDir("./templates/code/" + data.PackageName + "/controller/")
	file.MkDir("./templates/code/" + data.PackageName + "/models/")
	file.MkDir("./templates/code/" + data.PackageName + "/services/")
	file.MkDir("./templates/code/" + data.PackageName + "/vue/")
	file.MkDir("./templates/code/" + data.PackageName + "/js/")

	//写入文件
	file.FileCreate(controllerByte, "./templates/code/"+data.PackageName+"/controller/"+data.ClassName+"Controller.go")
	file.FileCreate(*bytes.NewBufferString(str), "./templates/code/"+data.PackageName+"/models/"+data.ClassName+".go")
	file.FileCreate(serviceByte, "./templates/code/"+data.PackageName+"/services/"+data.ClassName+".go")
	file.FileCreate(*bytes.NewBufferString(str2), "./templates/code/"+data.PackageName+"/vue/"+data.ClassName+".vue")
	file.FileCreate(jsByte, "./templates/code/"+data.PackageName+"/js/"+data.ClassName+".js")
